...
def _profile_skill_names(profile: Dict) -> List[str]:
    ...
    if isinstance(skills, dict):
        return [str(name) for name in skills.keys() if str(name).strip()]
...
def _maybe_audit_top_repo(
    repos: List[Dict[str, str]],
    *,
    audit_top_repo: bool,
    audit_expected_capabilities: List[str] | None,
    audit_allow_light_run: bool,
    audit_keep_workspace: bool,
) -> List[Dict[str, str]]:
    normalized = [dict(repo) for repo in repos]
    if not audit_top_repo or not normalized:
        return normalized

    top_repo = dict(normalized[0])
    try:
        audit_result = audit_repository(
            top_repo.get("url", ""),
            expected_capabilities=audit_expected_capabilities,
            allow_light_run=audit_allow_light_run,
            keep_workspace=audit_keep_workspace,
        )
        top_repo["audit_summary"] = {
            "verdict": audit_result.get("verdict", ""),
            "summary": audit_result.get("summary", ""),
            "highlights": audit_result.get("highlights", []),
            "usable_modules": audit_result.get("usable_modules", []),
            "run_summary": audit_result.get("run_summary", {}),
            "artifacts": audit_result.get("artifacts", {}),
        }
    except Exception as exc:  # pragma: no cover - network/runtime dependent
        logger.warning("Top repo 审计失败 %s: %s", top_repo.get("url", ""), exc)
        top_repo["audit_summary"] = {"verdict": "failed", "summary": f"仓库审计失败：{exc}"}

    normalized[0] = top_repo
    return normalized
...
def _llm_generate_queries(skill_gaps: List[str], profile: Dict, analysis: Dict) -> List[Dict]:
    ...
    if not has_llm_configured():
        raise RuntimeError("未配置可用的 LLM，无法生成搜索策略")

    try:
        prompt = _QUERY_GEN_PROMPT.format(
            experience_level=profile.get("experience_level", "未知"),
            user_skills="、".join(_profile_skill_names(profile)[:8]) or "暂无",
        ...
        raise RuntimeError(f"LLM 认证失败，请检查当前 provider 配置: {e}")
    except (APIConnectionError, APITimeoutError) as e:
        raise RuntimeError(f"无法连接到当前 LLM 服务: {e}")
...
def _plan_search_strategy(skill_gaps: List[str], profile: Dict, analysis: Dict) -> Dict:
    ...
    if not has_llm_configured():
        raise RuntimeError("未配置可用的 LLM，无法进行搜索规划")
    ...
    try:
        ...
        raise RuntimeError(f"无法连接到当前 LLM 服务，请检查网络或 provider 配置: {e}")
    except (json.JSONDecodeError, KeyError) as e:
...
def _llm_rerank(candidates: List[Dict], skill_gaps: List[str], profile: Dict, top_n: int) -> List[Dict]:
    ...
    if not has_llm_configured():
        raise RuntimeError("未配置可用的 LLM，无法进行项目重排序")
    ...
    try:
        ...
        selected = result.get("selected", [])[:top_n]
        normalized = []
...
def _llm_replan_search(previous_queries: List[str], failure_reason: str, skill_gaps: List[str], profile: Dict) -> Dict:
    ...
    if not has_llm_configured():
        raise RuntimeError("未配置可用的 LLM，无法重新规划搜索")
...
def smart_recommend_projects(
    ...
    if user_choice == "use_local":
        return {
            "status": "success",
            "repos": _maybe_audit_top_repo(
                recommend_projects(skill_gaps, top_n=top_n, profile=profile),
...
def recommend_projects(skill_gaps: List[str], top_n: int = 3, profile: Dict | None = None) -> List[Dict[str, str]]:
    ...
    if gap_set:
        ...
        if api_items:
            if profile and has_llm_configured():
                rerank_candidates = [
                    {
                        "name": item["full_name"],
                        "url": item["html_url"],
                ...
                reranked = _llm_rerank(rerank_candidates, skill_gaps, profile, top_n)
                if reranked:
                    return reranked
                raise RuntimeError("LLM 项目评分失败：未返回有效结果")
            results = []
    ...
    catalog_recommendations: List[Dict[str, str]] = []
    for _, overlap, repo in ranked:
        if overlap > 0:
        ...
        catalog_recommendations.append({
            "name": repo["name"],
            "url": repo["url"],
        ...
        rerank_candidates = [
            {
                ...
                "url": repo["url"],
                "stars": repo["stars"],
                "description": f"tags={', '.join(sorted(source_repo['tags']))}; difficulty={repo['difficulty']}; reason={repo['reason']}",
            }
            for repo, (_, _, source_repo) in zip(catalog_recommendations, ranked)
        ]
        reranked = _llm_rerank(rerank_candidates, skill_gaps, profile, top_n)
        if reranked:
            return reranked
        raise RuntimeError("LLM 项目评分失败：未返回有效结果")
    return catalog_recommendations[:top_n]
