...
from job_assistant.llm_runtime import complete_json, has_llm_configured
...
def analyze_jd(jd_text: str) -> Dict[str, Any]:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            raise RuntimeError("LLM 认证失败，请检查当前 provider 的 API key / Codex 配置")
        except (APITimeoutError, APIConnectionError) as exc:
            logger.warning("网络超时（第%d次），1秒后重试：%s", attempt, exc)
            ...
            if not json_retried:
                ...
                logger.warning("JSON 解析连续失败，降级到启发式分析：%s", exc)
                return _validate_result(_heuristic_analyze_jd(jd_text))
        except Exception as exc:
            logger.warning("LLM 分析未知错误（第%d次）：%s", attempt, exc)
            if attempt < max_retries:
                time.sleep(1)
